...
import { NIGHT_ROUTINE_LEVELS } from '../../data/levelData';
...
const NightRoutineActivity = ({ navigation, route }) => {
  const { user } = useAuth();
  const { activeChild } = useApp();
  const level = Math.min(route.params?.level || 1, 20);
  const activityId = route.params?.activityId || 'night';
  const data = NIGHT_ROUTINE_LEVELS[level - 1];
  ...
  useEffect(() => {
    ...
    speak("Drag the night routine steps to the correct slots!");
  }, [level]);
  ...
  const handleComplete = async () => {
    ...
    speak("Sweet dreams! Routine complete!");
  };

  const goNextLevel = () => {
    if (level < 20) {
      navigation.replace('NightRoutineActivity', { level: level + 1, activityId });
    } else {
      navigation.goBack();
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#1A1A2E', '#16213E']} style={styles.header}>
        ...
        <Text style={styles.levelTag}>NIGHT ROUTINE · LEVEL {level}</Text>
        <Text style={styles.title}>Sleepy Time Steps!</Text>
...
const styles = StyleSheet.create({
  ...
  targetSlot: { width: 85, height: 100, borderRadius: 20, borderStyle: 'dashed', borderWidth: 2, borderColor: '#4E4E6A', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(78, 78, 106, 0.1)' },
  ...
  slotNumber: { fontSize: 24, color: '#4E4E6A', fontWeight: '900' },
  ...
  resultCard: { backgroundColor: '#16213E', borderRadius: 30, padding: 30, alignItems: 'center', elevation: 5, marginTop: 50 },
  resultEmoji: { fontSize: 60, marginBottom: 10 },
  resultTitle: { fontSize: 24, fontWeight: '900', color: '#FFF', marginBottom: 6 },
...
export default NightRoutineActivity;
