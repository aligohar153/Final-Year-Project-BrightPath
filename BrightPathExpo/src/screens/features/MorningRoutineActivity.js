...
import { MORNING_ROUTINE_LEVELS } from '../../data/levelData';
import { speak } from '../../utils/ttsService';
import { useAuth } from '../../context/AuthContext';
import { useApp } from '../../context/AppContext';
import { completeLevel, awardSpecialBadge } from '../../utils/progressService';

const { width, height } = Dimensions.get('window');

const MorningRoutineActivity = ({ navigation, route }) => {
  const { user } = useAuth();
  const { activeChild } = useApp();
  const level = Math.min(route.params?.level || 1, 20);
  const activityId = route.params?.activityId || 'morning';
  const data = MORNING_ROUTINE_LEVELS[level - 1];
  ...
  useEffect(() => {
          ...
          onPanResponderRelease: (e, gesture) => {
            ...
            if (activeHoveredSlot !== null) {
              ...
              if (isCorrectPosition) {
                ...
                speak(step.text);
                Vibration.vibrate(50); // Success snap feedback
                
                // Use a functional update for sequence check logic
                ...
                setMistakes(m => m + 1);
                Vibration.vibrate([0, 30, 30, 30]); // Error feedback pattern
              }
            }
    ...
    speak("Drag the steps to the correct slots!");
  }, [level]);
  ...
  const handleComplete = async () => {
    ...
    if (mistakes === 0) {
      await awardSpecialBadge('Perfect Routine', user?.uid);
    }
    
    setTimeout(() => setPhase('result'), 1000);
    speak("Great morning routine!");
  };

  const goNextLevel = () => {
    if (level < 20) {
      navigation.replace('MorningRoutineActivity', { level: level + 1, activityId });
    } else {
      navigation.goBack();
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#FFD93D', '#FF8C00']} style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtn}>
          <Text style={styles.backText}>⬅️</Text>
        </TouchableOpacity>
        <Text style={styles.levelTag}>MORNING ROUTINE · LEVEL {level}</Text>
        <Text style={styles.title}>Order the Steps!</Text>
      ...
      <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
        {phase === 'play' ? (
          ...
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Routine Complete!</Text>
            <Text style={styles.resultScore}>+50 Stars Earned</Text>
            <TouchableOpacity style={styles.nextLevelBtn} onPress={goNextLevel}>
              <LinearGradient colors={['#FFD93D', '#FF8C00']} style={styles.nextLevelGrad}>
...
const styles = StyleSheet.create({
  ...
  targetSlot: { width: 85, height: 100, borderRadius: 20, borderStyle: 'dashed', borderWidth: 2, borderColor: '#FFD93D', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255, 217, 61, 0.1)' },
  ...
  slotNumber: { fontSize: 24, color: '#FFB800', fontWeight: '900' },
  ...
  resultTitle: { fontSize: 24, fontWeight: '900', color: '#FF8C00', marginBottom: 6 },
...
export default MorningRoutineActivity;
