...
import { CLEANUP_LEVELS } from '../../data/levelData';
import { speak } from '../../utils/ttsService';
import { useAuth } from '../../context/AuthContext';
import { useApp } from '../../context/AppContext';
import { completeLevel } from '../../utils/progressService';

const OceanCleanupActivity = ({ navigation, route }) => {
  const { user } = useAuth();
  const { activeChild } = useApp();
  const level = Math.min(route.params?.level || 1, 20);
  const activityId = route.params?.activityId || 'ocean';
  const data = CLEANUP_LEVELS[level - 1];
  ...
  useEffect(() => {
    ...
    speak(`We found a ${data.item}! Is it Trash or a Sea Animal?`);
  }, [level]);
  ...
  const handleChoice = async (idx) => {
    ...
    if (idx === data.correct) {
      setScore(s => s + 50);
      speak(`Correct! ${data.item} belongs in the ${data.choices[idx]}. ${data.tip}`);
      await completeLevel(activityId, level, activeChild?.id || user?.uid);
      setPhase('result');
    } else {
      speak("Wait! Check again.");
  ...
  const goNextLevel = () => {
    if (level < 20) {
      navigation.replace('OceanCleanupActivity', { level: level + 1, activityId });
    } else {
      navigation.goBack();
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#0077B6', '#023E8A']} style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtn}>
          <Text style={styles.backText}>⬅️</Text>
        </TouchableOpacity>
        <Text style={styles.levelTag}>OCEAN CLEANUP · LEVEL {level}</Text>
        <Text style={styles.title}>Save the Ocean!</Text>
      ...
      <Animated.ScrollView contentContainerStyle={styles.content} style={{ opacity: fadeAnim }}>
        {phase === 'quiz' ? (
          <View style={styles.quizCard}>
            ...
            <Text style={styles.itemName}>{data.item}</Text>
            <Text style={styles.questionText}>Is it Trash or a Sea Animal?</Text>
          ...
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Ocean Hero!</Text>
            ...
            <TouchableOpacity style={styles.nextLevelBtn} onPress={goNextLevel}>
              <LinearGradient colors={['#0077B6', '#023E8A']} style={styles.nextLevelGrad}>
...
const styles = StyleSheet.create({
  ...
  itemEmoji: { fontSize: 90, marginBottom: 10 },
  itemName: { fontSize: 32, fontWeight: '900', color: '#03045E', marginBottom: 5 },
  questionText: { fontSize: 16, fontWeight: '700', color: COLORS.textMedium, marginBottom: 30 },
  choices: { flexDirection: 'row', gap: 15, width: '100%' },
  choiceBtn: { flex: 1, backgroundColor: '#F8F9FA', padding: 22, borderRadius: 20, borderWidth: 2, borderColor: '#EEE', alignItems: 'center' },
  choiceCorrect: { borderColor: '#6BCB77', backgroundColor: '#F0FFF4' },
  choiceWrong: { borderColor: '#FF6B6B', backgroundColor: '#FFF5F5' },
  choiceText: { fontSize: 16, fontWeight: '900', color: COLORS.textMedium },
...
export default OceanCleanupActivity;
