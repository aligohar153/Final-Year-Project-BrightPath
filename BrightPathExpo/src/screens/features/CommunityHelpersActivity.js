...
import { HELPERS_LEVELS } from '../../data/levelData';
import { speak } from '../../utils/ttsService';
import { useAuth } from '../../context/AuthContext';
import { useApp } from '../../context/AppContext';
import { completeLevel } from '../../utils/progressService';

const CommunityHelpersActivity = ({ navigation, route }) => {
  const { user } = useAuth();
  const { activeChild } = useApp();
  const level = Math.min(route.params?.level || 1, 20);
  const activityId = route.params?.activityId || 'helpers';
  const data = HELPERS_LEVELS[level - 1];
  ...
  const goNextLevel = () => {
    if (level < 20) {
      navigation.replace('CommunityHelpersActivity', { level: level + 1, activityId });
    } else {
      navigation.goBack();
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#2B2D42', '#8D99AE']} style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtn}>
          <Text style={styles.backText}>⬅️</Text>
        </TouchableOpacity>
        <Text style={styles.levelTag}>COMMUNITY HELPERS · LEVEL {level}</Text>
        ...
        {phase === 'quiz' ? (
          ...
          <View style={styles.resultCard}>
            ...
            <TouchableOpacity style={styles.nextLevelBtn} onPress={goNextLevel}>
              <LinearGradient colors={['#2B2D42', '#8D99AE']} style={styles.nextLevelGrad}>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#EDF2FB' },
  ...
  resultTitle: { fontSize: 24, fontWeight: '900', color: '#2B2D42', marginBottom: 10 },
...
export default CommunityHelpersActivity;
