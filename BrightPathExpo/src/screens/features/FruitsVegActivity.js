...
import { FOOD_LEVELS } from '../../data/levelData';
import { speak } from '../../utils/ttsService';
import { useAuth } from '../../context/AuthContext';
import { useApp } from '../../context/AppContext';
import { completeLevel } from '../../utils/progressService';

const FruitsVegActivity = ({ navigation, route }) => {
  const { user } = useAuth();
  const { activeChild } = useApp();
  const level = Math.min(route.params?.level || 1, 20);
  const activityId = route.params?.activityId || 'food';
  const data = FOOD_LEVELS[level - 1];
  ...
  useEffect(() => {
    ...
    speak(`Is ${data.name} a Fruit or a Vegetable?`);
  }, [level]);
  ...
  const handleChoice = async (idx) => {
    ...
    if (idx === data.correct) {
      setScore(s => s + 50);
      speak(`Correct! ${data.name} is a ${data.choices[idx]}. ${data.tip}`);
  ...
  const goNextLevel = () => {
    if (level < 20) {
      navigation.replace('FruitsVegActivity', { level: level + 1, activityId });
    } else {
      navigation.goBack();
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#99D98C', '#52B69A']} style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtn}>
          <Text style={styles.backText}>⬅️</Text>
        </TouchableOpacity>
        <Text style={styles.levelTag}>FRUITS & VEGGIES · LEVEL {level}</Text>
        <Text style={styles.title}>Healthy Eating!</Text>
      ...
      <Animated.ScrollView contentContainerStyle={styles.content} style={{ opacity: fadeAnim }}>
        {phase === 'quiz' ? (
          <View style={styles.quizCard}>
            <Text style={styles.foodEmoji}>{data.emoji}</Text>
            <Text style={styles.foodName}>{data.name}</Text>
            <Text style={styles.questionText}>Is it a Fruit or Vegetable?</Text>
          ...
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Yummy!</Text>
            ...
            <TouchableOpacity style={styles.nextLevelBtn} onPress={goNextLevel}>
              <LinearGradient colors={['#99D98C', '#52B69A']} style={styles.nextLevelGrad}>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#F1FAE5' },
  ...
  levelTag: { fontSize: 10, fontWeight: '900', color: '#1A759F', letterSpacing: 1.5, marginBottom: 4 },
  ...
  foodEmoji: { fontSize: 90, marginBottom: 10 },
  foodName: { fontSize: 32, fontWeight: '900', color: '#1E6091', marginBottom: 5 },
  ...
  resultTitle: { fontSize: 24, fontWeight: '900', color: '#1E6091', marginBottom: 10 },
...
export default FruitsVegActivity;
