...
import { EATING_LEVELS } from '../../data/levelData';
import { speak } from '../../utils/ttsService';
import { useAuth } from '../../context/AuthContext';
import { useApp } from '../../context/AppContext';
import { completeLevel } from '../../utils/progressService';

const HealthyEatingActivity = ({ navigation, route }) => {
  const { user } = useAuth();
  const { activeChild } = useApp();
  const level = Math.min(route.params?.level || 1, 20);
  const activityId = route.params?.activityId || 'eating';
  const data = EATING_LEVELS[level - 1];
  ...
  useEffect(() => {
    ...
    speak(`Is an ${data.food} Healthy or Junk Food?`);
  }, [level]);
  ...
  const handleChoice = async (idx) => {
    ...
    if (idx === data.correct) {
      setScore(s => s + 50);
      speak(`Correct! ${data.food} is ${data.choices[idx]}. ${data.tip}`);
      await completeLevel(activityId, level, activeChild?.id || user?.uid);
      setPhase('result');
    } else {
      speak("Wait! Think about it again.");
  ...
  const goNextLevel = () => {
    if (level < 20) {
      navigation.replace('HealthyEatingActivity', { level: level + 1, activityId });
    } else {
      navigation.goBack();
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#8BC34A', '#689F38']} style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backBtn}>
          <Text style={styles.backText}>⬅️</Text>
        </TouchableOpacity>
        <Text style={styles.levelTag}>HEALTHY EATING · LEVEL {level}</Text>
        <Text style={styles.title}>Strong Body!</Text>
      ...
      <Animated.ScrollView contentContainerStyle={styles.content} style={{ opacity: fadeAnim }}>
        {phase === 'quiz' ? (
          <View style={styles.quizCard}>
            ...
            <Text style={styles.questionText}>Is it Healthy or Junk Food?</Text>
          ...
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Great Choice!</Text>
            ...
            <TouchableOpacity style={styles.nextLevelBtn} onPress={goNextLevel}>
              <LinearGradient colors={['#8BC34A', '#689F38']} style={styles.nextLevelGrad}>
...
const styles = StyleSheet.create({
  ...
  foodName: { fontSize: 32, fontWeight: '900', color: '#33691E', marginBottom: 5 },
  ...
  choiceText: { fontSize: 18, fontWeight: '900', color: COLORS.textMedium },
  ...
  resultTitle: { fontSize: 24, fontWeight: '900', color: '#33691E', marginBottom: 10 },
...
export default HealthyEatingActivity;
